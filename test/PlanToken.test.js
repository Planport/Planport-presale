...
describe('PlanToken', () => {
    ...
    it('transfer token from one account to another account', async()=>{
        ...
        const transfer2 = await planToken.methods.transfer(accounts[2], web3.utils.toWei('100', 'ether')).send({
            from: accounts[1],
            gas: '1000000'
        });

        const balance = await planToken.methods.balanceOf(accounts[2]).call();
        assert.equal('100', web3.utils.fromWei(balance, 'ether'));
    });

    it('cant transfer tokens if an account doesnt have tokens', async()=>{
        try {
            const transfer = await planToken.methods.transfer(accounts[2], web3.utils.toWei('100', 'ether')).send({
                from: accounts[3],
                gas: '1000000'
            });
            assert(false);
